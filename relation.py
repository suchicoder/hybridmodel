...
model1 = Word2Vec.load('newmodel1.bin')
print(model1)
...
def find_relation_elements(text, classs, method, attribute, association):
    find=0
    subject = ''
    relationlist = []
    attributelist = []
            ...
            if n == asubject or n == averb:
                for x in classs:
                    if x == asubject or x == averb:
                        attributelist.append(x + '-' + n) 
                        break  
    return (relationlist, attributelist)
...
files_path = "/home/ubuntu/crfnew/xml1"
...
def word2features(doc, i):
    ...
    if i > 0:
        word1 = doc[i-1][0]
        postag1 = doc[i-1][1]
        word_tag1 = doc[i-1][0]+'_'+doc[i-1][1]
        label1 = find_labels(word1)
        centroid1 = find_centroid(word1)
        tag_centroid1 = find_tagcentroid(word_tag1) 
        features.extend([
            ...
            '-1:postag=' + postag1,
            '-1:label=' + label1,
            '-1:centroid=' + str(centroid1),
            '-1:tag_centroid=' + str(tag_centroid1),
...
def extract_features(doc1):
    stop_words = set(stopwords.words('english'))
    doc = []
    for r in doc1:
        if not r[0] in stop_words:
            doc.append(r)
...
for train, test in kf.split(X):
    ...
    for xi, yi, zi, readi  in zip(X_test, y_test, z_test, read_files_test): 
        ...
        relationlist, attributelist = find_relation_elements(zi, classs1, method1, attribute1, association1)
        f1.write(readi)
        f1.write('\n')
        f1.write("relation*******************")
        f1.write('\n')
    ...
    labels = {"class": 0, "attribute": 1, "association": 2, "method": 3, "no": 4}
    ...
    f.write(classification_report(truths, predictions, target_names=["class", "attribute", "association", "method", "no"]))
